...
/// Used where SMS.online names differ significantly from ISO standard names.
static NAME_OVERRIDES: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    HashMap::from([
        // Primary mappings
        ("usa", "US"),
        ("united states", "US"),
...
static ISO_NAME2ALPHA2: LazyLock<HashMap<String, Alpha2>> = LazyLock::new(|| {
...
/// Built from sms_online_countries.json at startup.
pub static SMS_ONLINE_ID2COUNTRY: LazyLock<HashMap<u16, Country>> = LazyLock::new(|| {
    let raw: HashMap<String, Value> =
        serde_json::from_str(COUNTRIES_JSON).expect("sms_online_countries.json is invalid");
    ...
    for (id_str, name_val) in raw {
        ...
        #[cfg(feature = "tracing")]
        tracing::debug!("No ISO match for SMS.online country name: '{name}' (id={id})");
    }
...
pub static COUNTRY2SMS_ONLINE_ID: LazyLock<HashMap<String, u16>> = LazyLock::new(|| {
    let mut m = HashMap::with_capacity(SMS_ONLINE_ID2COUNTRY.len());
    for (id, country) in SMS_ONLINE_ID2COUNTRY.iter() {
...
mod tests {
    ...
    fn test_country_to_sms_online_id() {
        // Test countries from sms_online_countries.json
